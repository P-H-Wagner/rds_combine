...
TGraph* bestFit(TTree *t, TString x, TString y) {
    t->Draw(y+":"+x, "quantileExpected == -1"); //adapt to minus 1!!!!
...
TGraph* contourPlot(TTree *t, TString x, TString y, double pmin, double pmax, TGraph *bestFit) {
    ...
    int m = gr->GetN();
    for (int i = 0; i < m; ++i) { xi[i] -= x0; yi[i] -= y0; }
...
void threeContours(TString name, TString x, TString y, TH2 *frame) {
    TFile *_file68 = TFile::Open("higgsCombine_68.MultiDimFit.mH120.root");
    TFile *_file95 = TFile::Open("higgsCombine_95.MultiDimFit.mH120.root");
    TFile *_file99 = TFile::Open("higgsCombine_99.MultiDimFit.mH120.root");
    TTree *t68 = (TTree*) _file68->Get("limit");
    TTree *t95 = (TTree*) _file95->Get("limit");
    TTree *t99 = (TTree*) _file99->Get("limit");
...
void contourPlot(const char *name, const char *title, double qqHmax=10., double ggHmax=4.) {
    std::cout << "qqHmax = " << qqHmax << ", ggHmax = " << ggHmax << std::endl;
    std::cout << "name = " << name << ", title = " << title << std::endl;
    TFile *fOut = TFile::Open(Form("plots/%s.root", name), "RECREATE");
    //TH2F *frame = new TH2F(name, Form("%s;#sigma/#sigma_{SM} pp #rightarrow qqH;#sigma/#sigma_{SM} gg #rightarrow H;",title), 30, 0, 2, 30, 0, 2);
    TH2F *frame = new TH2F(name, Form("68 - 95 - 99 CL; R(Ds) ;R(Ds*);"), 30, 0, 2, 30, 0, 2);
    TCanvas *c1 = new TCanvas("c1","c1");
    gStyle->SetOptStat(0);
    threeContours(name, "rDs", "rDsStar", frame);
    c1->Print(Form("plots/%s.pdf", name));
